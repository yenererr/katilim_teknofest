...
import { BANKALAR, UCRETLER } from '../data/piyasa';
import { BankMark } from './BankMark';
import { tlBicim2 } from '../lib/finansman';

/** Ücret matrisi: satırlar ücret kalemi, sütunlar banka. Statik örnek tarife yok. */
export const FeesView: React.FC = () => (
  <div className="space-y-4">
      ...
      {UCRETLER.length === 0 ? (
        <div className="px-4 py-10 text-center">
          <p className="text-sm text-txt-secondary">
            Canlı ücret tarifesi henüz bağlı değil. Örnek veya tahmini rakam gösterilmiyor.
          </p>
          <p className="mt-2 text-xs text-txt-muted">
            Güncel FAST, EFT ve aidat bilgisi için ilgili bankanın ücret sayfasını kontrol edin.
          </p>
        </div>
      ) : (
        <div
    ...
    <p className="flex items-start gap-2 px-1 text-xs leading-relaxed text-txt-muted">
      ...
      Yanlış bilgi vermemek için doğrulanmamış ücret listesi tutulmaz. Nihai tarife için bankanın
      kendi ücret sayfasını doğrulayın.
    </p>
  </div>
);
