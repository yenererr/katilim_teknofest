...
import { Sparkles, AlertTriangle, Layers, FileCode2, RefreshCw, X } from 'lucide-react';
...
function AppInner() {
  ...
  return (
    <div className="flex min-h-dvh bg-canvas text-txt">
      <Sidebar
        activeTab={activeTab}
        setActiveTab={setActiveTab}
        extractedCount={ogeler.length}
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <main className="flex-1 px-4 pt-5 pb-20 sm:px-6 lg:px-8 lg:pb-8">
          <AnimatePresence>
            {error && (
              <motion.div
              ...
              >
                <div className="flex min-w-0 items-start gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="mt-0.5 leading-relaxed text-risk-800 dark:text-risk-200">{error}</p>
                  </div>
                </div>
          ...
          {activeTab === 'single' && (
            <div {...panelProps('single')} className="space-y-5">
        ...
        <footer className="hidden border-t border-line bg-surface py-4 lg:block">
          <div className="flex flex-col items-center justify-between gap-2 px-8 text-xs text-txt-secondary sm:flex-row">
            <span>© 2026 Katılım Bankacılığı Bilgi Çıkarım Ajanı</span>
            <span className="font-mono text-txt-muted">
              kar_payi_orani · vade_ay · tahsis_ucreti · urun_turu
            </span>
          </div>
        </footer>
      </div>
